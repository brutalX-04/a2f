...
class lisensi:
	...
	def join(self):
		try:
			...
			headers = {'authority': 'a2flisensi.000webhostapp.com', 'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7', 'accept-language': 'en-US,en;q=0.9,id;q=0.8', 'cache-control': 'max-age=0', 'content-type': 'application/x-www-form-urlencoded', 'origin': 'https://a2flisensi.000webhostapp.com', 'referer': 'https://a2flisensi.000webhostapp.com/', 'sec-ch-ua': '"Chromium";v="116", "Not)A;Brand";v="24", "Google Chrome";v="116"', 'sec-ch-ua-mobile': '?0', 'sec-ch-ua-platform': '"Linux"', 'sec-fetch-dest': 'document', 'sec-fetch-mode': 'navigate', 'sec-fetch-site': 'same-origin', 'sec-fetch-user': '?1', 'upgrade-insecure-requests': '1', 'user-agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Safari/537.36'}
			post = rs.post('https://a2flisensi.000webhostapp.com/insert.php', headers=headers, data=data).text
			if 'Succes Insert Data' in post:
				self.check()

			else:
				print('%sUser Join Failled%s'%(m,p))
	...
	def update(self):
		try:
			...
			get = rs.post('https://a2flisensi.000webhostapp.com/getuser.php', headers=headers, data=data).text
			status = re.search('"STATUS":"(.*?)"', get).group(1)
			limit = int(re.search('"LIMIT":"(.*?)"', get).group(1)) - 1
			data = {
			    'uuid': uid,
			    'status': status,
			    'limit': limit,
			}
			post = rs.post('https://a2flisensi.000webhostapp.com/update.php', headers=headers, data=data).text

			if 'Succes Update User Data' in post:
				pass

			else:
				print('%sFailled Update Data%s'%(m,p))
...
class cookie:
	...
	def get_cookies(self,acc):
		try:
			...
			print('%sFailled Get Cookies%s'%(m,p))
...
class auth:
	...
	def password(self):
		...
		try:
			...
			print('%sFailled Handle Password%s'%(m,p))
	...
	def confirm(self):
		try:
			...
			print('%sFailled Confirm OTP%s'%(m,p))
